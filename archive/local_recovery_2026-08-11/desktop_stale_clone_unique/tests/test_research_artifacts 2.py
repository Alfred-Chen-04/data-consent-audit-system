...
def test_ssrp_results_tables_exist_and_use_current_research_package() -> None:
    ...
    assert "- Latest target tiers: High-Risk=5" in text
    assert "| Coca-Cola | food | High-Risk | fail | Accept | reject\\|customize\\|dismiss | not scored | missing | missing |" in text
...
def test_ssrp_writing_pack_exists_and_marks_ready_claims() -> None:
    ...
    assert "Latest target tiers" in text
...
def test_ssrp_claim_register_exists_and_labels_claim_statuses() -> None:
    ...
    assert "| C4 | RQ1 | Latest target tiers are High-Risk=5. | Ready |" in text
...
def test_ssrp_poster_plan_exists_and_marks_week2_gate_ready() -> None:
    ...
    assert "| RQ1 evidence | Results band | Current target tiers: High-Risk=5. |" in text
