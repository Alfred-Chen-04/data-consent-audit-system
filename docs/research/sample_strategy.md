...
Current paper-facing exports contain 37 audit-report rows and 15 longitudinal
...
The first expanded weekly capture completed for all five targets. Coca-Cola
remained `Compliant` with Accept, Reject, and Customize visible in the latest
capture. The longitudinal export now includes Coca-Cola as a severity A
no-change observation with `event_count=0`, so stability is represented in RQ2
rather than disappearing from the weekly summary table.
...
The Week 2 sanity check is the post-capture evidence gate. It currently records
the pre-run baseline for cohort `week2-2026-06-06`: 0 of 5 frozen targets have a
Week 2 consent-table row yet. After the capture run, rerun `week2-sanity-check`
and require every target to have a consent row, screenshot/DOM/hash evidence,
and matching audit-report evidence before treating the run as complete.
...
`pending_capture`. Its Run Controls section includes the dry-run and live
...
package has 37 audit reports and 15 longitudinal summaries, and the CMP
