...
**Status: `pre_freeze`. This is not a final index and does not record an
advisor response or project fallback.**
...
| Presentation | [10-slide presentation PPTX](presentation/ssrp_consent_audit_presentation_draft_2026-07-22.pptx) and [montage](presentation/ssrp_consent_audit_presentation_draft_2026-07-22_montage.png) | Current draft; revise only after selecting the response branch |
| Poster | [Aligned poster PPTX](poster/ssrp_poster_aligned_review_2026-07-25.pptx), [PDF](poster/ssrp_poster_aligned_review_2026-07-25.pdf), and [PNG](poster/ssrp_poster_aligned_review_2026-07-25.png) | Current joint-review version; not final |
...
|---|---|---|
| July 26-29 | Send or discuss the joint packet and record only actual answers received | Joint decision rows contain exact answer, `confirmed` status, reviewer, and date |
| July 29, 23:59 Asia/Shanghai | Internal project cutoff; not an advisor deadline | Cutoff has passed or actual responses are already recorded |
| July 30 | Select actual-response or no-response basis for each joint decision | Matrix rows use a validated response basis; advisor fields remain blank for project fallbacks |
| July 30-August 2 | Apply the selected values to every mapped presentation, poster, and evidence surface | All 20 matrix rows reach `applied_verified` with executor and timezone-aware timestamp |
...
- [ ] One response basis is valid for every revision row.
- [ ] All 20 expected revision IDs are present and `applied_verified`.
- [ ] Presentation wording, overflow, render, montage, and rehearsal checks pass.
- [ ] Poster wording, overflow, 48 x 36 print dimensions, PDF/PNG render, and
  visual inspection pass.
- [ ] Evidence tables, current/missing refs, limitations, packages, and hashes
  are refreshed from the revised sources.
- [ ] Full tests, Ruff, Mypy, compileall, links, JSON, ZIP, and Git-state checks
  pass.
- [ ] The regenerated manifest reports `ready_for_final_freeze=true` and no
  blocker.
